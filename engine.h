    ...
    for (int i = 0; i <= 7; i++)
        for (int j = 0; j <= 7; j++)
            if (pboard[i][j] != 0)
            {
                ...
                if (pboard[i][j] == 2)
                    score = score + 325; //Bishop equals 325
                if (pboard[i][j] == 3)
                    score = score + 320; //knight equals 320
                if (pboard[i][j] == 4)
                    score = score + 500; //Rook equals 500
                if (pboard[i][j] == 5)
                    score = score + 975; //Queen equals 975
                if (pboard[i][j] == 6)
                    score = score + 20000; //King as much as possible obviously
