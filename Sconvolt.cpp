...
#include "variables.h" //where all the bitmaps, fonts and other variables are defined
    ...
    while (Exit == false)
    {
        ...
        if (mode == 0)
            al_draw_tinted_bitmap(about, color, 785, 537, NULL);

        if (mode == 0)
        { ///Mode 0, it's play mode
        ...
        if (game1.user_depth == 1)
            al_draw_tinted_bitmap(b1, color2, 650, 120, NULL);
        else
            al_draw_tinted_bitmap(b1, color, 650, 120, NULL);
        if (game1.user_depth == 2)
            al_draw_tinted_bitmap(b2, color2, 710, 120, NULL);
        else
            al_draw_tinted_bitmap(b2, color, 710, 120, NULL);
        if (game1.user_depth == 3)
            al_draw_tinted_bitmap(b3, color2, 770, 120, NULL);
        else
            al_draw_tinted_bitmap(b3, color, 770, 120, NULL);
        if (game1.user_depth == 4)
            al_draw_tinted_bitmap(b4, color2, 830, 120, NULL);
        else
            al_draw_tinted_bitmap(b4, color, 830, 120, NULL);
        if (game1.user_depth == 5)
            al_draw_tinted_bitmap(b5, color2, 890, 120, NULL);
        else
            al_draw_tinted_bitmap(b5, color, 890, 120, NULL);
        ...
        { ///Mode 1, it's editor mode
            ...
            if (game1.user_side == 1 && game1.human_vs_human == false)
                al_draw_tinted_bitmap(whiteb, color2, 657, 430, NULL);
            else
                al_draw_tinted_bitmap(whiteb, color, 657, 430, NULL);
            if (game1.user_side == 0 && game1.human_vs_human == false)
                al_draw_tinted_bitmap(blackb, color2, 657 + 120, 430, NULL);
            else
                al_draw_tinted_bitmap(blackb, color, 657 + 120, 430, NULL);
            if (game1.human_vs_human == true)
                al_draw_tinted_bitmap(no_cpu, color2, 657 + 120 + 105, 430, NULL);
            else
                al_draw_tinted_bitmap(no_cpu, color, 657 + 120 + 105, 430, NULL);

            al_draw_text(font2, al_map_rgb(255, 255, 255), board_size_x + 5, 495, NULL, "Who starts first?");
            if (game1.whostartsfirst == 1)
                al_draw_tinted_bitmap(whiteb, color2, 657, 533, NULL);
            else
                al_draw_tinted_bitmap(whiteb, color, 657, 533, NULL);
            if (game1.whostartsfirst == 0)
                al_draw_tinted_bitmap(blackb, color2, 657 + 120, 533, NULL);
            else
                al_draw_tinted_bitmap(blackb, color, 657 + 120, 533, NULL);
        ...
        for (int f1 = 0; f1 <= 7; f1++)
        {
            ...
            {
                if (game1.bb[f2][f1] == 1)
                    al_draw_scaled_bitmap(white_pawn, 0, 0, al_get_bitmap_width(white_pawn), al_get_bitmap_height(white_pawn), (board_size_x / 8) * f1 + 10, (board_size_y / 8) * f2 + 10, 60, 60, 0);
                if (game1.bb[f2][f1] == -1)
                    al_draw_scaled_bitmap(black_pawn, 0, 0, al_get_bitmap_width(black_pawn), al_get_bitmap_height(black_pawn), (board_size_x / 8) * f1 + 10, (board_size_y / 8) * f2 + 10, 60, 60, 0);
                if (game1.bb[f2][f1] == 4)
                    al_draw_scaled_bitmap(white_rook, 0, 0, al_get_bitmap_width(white_rook), al_get_bitmap_height(white_rook), (board_size_x / 8) * f1 + 10, (board_size_y / 8) * f2 + 10, 60, 60, 0);
                if (game1.bb[f2][f1] == -4)
                    al_draw_scaled_bitmap(black_rook, 0, 0, al_get_bitmap_width(black_rook), al_get_bitmap_height(black_rook), (board_size_x / 8) * f1 + 10, (board_size_y / 8) * f2 + 10, 60, 60, 0);
                if (game1.bb[f2][f1] == 3)
                    al_draw_scaled_bitmap(white_knight, 0, 0, al_get_bitmap_width(white_knight), al_get_bitmap_height(white_knight), (board_size_x / 8) * f1 + 10, (board_size_y / 8) * f2 + 10, 60, 60, 0);
                if (game1.bb[f2][f1] == -3)
                    al_draw_scaled_bitmap(black_knight, 0, 0, al_get_bitmap_width(black_knight), al_get_bitmap_height(black_knight), (board_size_x / 8) * f1 + 10, (board_size_y / 8) * f2 + 10, 60, 60, 0);
                if (game1.bb[f2][f1] == 2)
                    al_draw_scaled_bitmap(white_bishop, 0, 0, al_get_bitmap_width(white_bishop), al_get_bitmap_height(white_bishop), (board_size_x / 8) * f1 + 10, (board_size_y / 8) * f2 + 10, 60, 60, 0);
                if (game1.bb[f2][f1] == -2)
                    al_draw_scaled_bitmap(black_bishop, 0, 0, al_get_bitmap_width(black_bishop), al_get_bitmap_height(black_bishop), (board_size_x / 8) * f1 + 10, (board_size_y / 8) * f2 + 10, 60, 60, 0);
                if (game1.bb[f2][f1] == 5)
                    al_draw_scaled_bitmap(white_queen, 0, 0, al_get_bitmap_width(white_queen), al_get_bitmap_height(white_queen), (board_size_x / 8) * f1 + 10, (board_size_y / 8) * f2 + 10, 60, 60, 0);
                if (game1.bb[f2][f1] == 6)
                    al_draw_scaled_bitmap(white_king, 0, 0, al_get_bitmap_width(white_king), al_get_bitmap_height(white_king), (board_size_x / 8) * f1 + 10, (board_size_y / 8) * f2 + 10, 60, 60, 0);
                if (game1.bb[f2][f1] == -5)
                    al_draw_scaled_bitmap(black_queen, 0, 0, al_get_bitmap_width(black_queen), al_get_bitmap_height(black_queen), (board_size_x / 8) * f1 + 10, (board_size_y / 8) * f2 + 10, 60, 60, 0);
                if (game1.bb[f2][f1] == -6)
                    al_draw_scaled_bitmap(black_king, 0, 0, al_get_bitmap_width(black_king), al_get_bitmap_height(black_king), (board_size_x / 8) * f1 + 10, (board_size_y / 8) * f2 + 10, 60, 60, 0);
            }
        }
        ...
        if (al_is_event_queue_empty(event_queue))
        {
            ...
            if (mouse_button_is_down)
            {
                ...
                int ty;
                ty = (mouse_from_y / (board_size_y / 8));
        ...
        if (Event.type == ALLEGRO_EVENT_MOUSE_BUTTON_UP)
        {
            ...
            {
                {
                    game1.bb[temp_to_y][temp_to_x] = piece_selected;
                    piece_selected = -99;
                    holding_piece = false;
                }
            }
            ...
            if (mode == 0)
            {
                ...
                if (game1.whostartsfirst != game1.user_side && game1.gamestarted == false && game1.human_vs_human == false)
                {
                    ...
                    if (game1.user_side == 1)
                        ...
                        game1.generateMove(game1.user_depth, true);
                ...
                */
                    ...
                    if (mode == 0 && game1.game_terminated == false)
                    {
                        ...
                        if (test == 3)
                        {
                            cout << "GAME FINISHED IN DRAW" << endl;
                            game1.who_won = 3;
                        }
...
*/
        ...
        if (Event.type == ALLEGRO_EVENT_MOUSE_BUTTON_DOWN) ///check if a mouse button was clicked
        {
            ...
            if (is_inside_button(mouse_from_x, mouse_from_y, 652, 121, 695, 157))
                game1.user_depth = 1;
            if (is_inside_button(mouse_from_x, mouse_from_y, 712, 121, 760, 157))
                game1.user_depth = 2;
            if (is_inside_button(mouse_from_x, mouse_from_y, 772, 121, 820, 157))
                game1.user_depth = 3;
            if (is_inside_button(mouse_from_x, mouse_from_y, 832, 121, 880, 157))
                game1.user_depth = 4;
            if (is_inside_button(mouse_from_x, mouse_from_y, 892, 121, 940, 157))
                game1.user_depth = 5;
            ...
            if (is_inside_button(mouse_from_x, mouse_from_y, 908, 483, 987, 516))
                Exit = true;
            ...
            if (mode == 1) //Editor mode
            {
                ...
                if (is_inside_button(mouse_from_x, mouse_from_y, 650, 220, 650 + 60, 220 + 60))
                {
                    holding_piece = true;
                    piece_on_hold = white_rook;
                    piece_selected = 4;
                }
                if (is_inside_button(mouse_from_x, mouse_from_y, 710, 220, 710 + 60, 220 + 60))
                {
                    holding_piece = true;
                    piece_on_hold = white_knight;
                    piece_selected = 3;
                }
                if (is_inside_button(mouse_from_x, mouse_from_y, 770, 220, 770 + 60, 220 + 60))
                {
                    holding_piece = true;
                    piece_on_hold = white_bishop;
                    piece_selected = 2;
                }
                if (is_inside_button(mouse_from_x, mouse_from_y, 830, 220, 830 + 60, 220 + 60))
                {
                    holding_piece = true;
                    piece_on_hold = white_pawn;
                    piece_selected = 1;
                }
                if (is_inside_button(mouse_from_x, mouse_from_y, 890, 220, 950 + 60, 220 + 60))
                {
                    holding_piece = true;
                    piece_on_hold = white_queen;
                    piece_selected = 5;
                }
                if (is_inside_button(mouse_from_x, mouse_from_y, 950, 220, 950 + 60, 220 + 60))
                {
                    holding_piece = true;
                    piece_on_hold = white_king;
                    piece_selected = 6;
                }

                if (is_inside_button(mouse_from_x, mouse_from_y, 650, 300, 650 + 60, 300 + 60))
                {
                    holding_piece = true;
                    piece_on_hold = black_rook;
                    piece_selected = -4;
                }
                if (is_inside_button(mouse_from_x, mouse_from_y, 710, 300, 710 + 60, 300 + 60))
                {
                    holding_piece = true;
                    piece_on_hold = black_knight;
                    piece_selected = -3;
                }
                if (is_inside_button(mouse_from_x, mouse_from_y, 770, 300, 770 + 60, 300 + 60))
                {
                    holding_piece = true;
                    piece_on_hold = black_bishop;
                    piece_selected = -2;
                }
                if (is_inside_button(mouse_from_x, mouse_from_y, 830, 300, 830 + 60, 300 + 60))
                {
                    holding_piece = true;
                    piece_on_hold = black_pawn;
                    piece_selected = -1;
                }
                if (is_inside_button(mouse_from_x, mouse_from_y, 890, 300, 890 + 60, 300 + 60))
                {
                    holding_piece = true;
                    piece_on_hold = black_queen;
                    piece_selected = -5;
                }
                if (is_inside_button(mouse_from_x, mouse_from_y, 950, 300, 950 + 60, 300 + 60))
                {
                    holding_piece = true;
                    piece_on_hold = black_king;
                    piece_selected = -6;
                }

                if (is_inside_button(mouse_from_x, mouse_from_y, 661, 433, 755, 466))
                {
                    game1.user_side = 1;
                    game1.human_vs_human = false;
                }
                if (is_inside_button(mouse_from_x, mouse_from_y, 781, 433, 867, 466))
                {
                    game1.user_side = 0;
                    game1.human_vs_human = false;
                }

                if (is_inside_button(mouse_from_x, mouse_from_y, 890, 433, 990, 466))
                    game1.human_vs_human = true;

                if (is_inside_button(mouse_from_x, mouse_from_y, 661, 537, 755, 570))
                {
                    game1.whostartsfirst = 1;
                }
                if (is_inside_button(mouse_from_x, mouse_from_y, 781, 536, 867, 570))
                {
                    game1.whostartsfirst = 0;
                }

                if (is_inside_button(mouse_from_x, mouse_from_y, 821, 381, 1024, 417))
                    game1.reset();
